...
def load_and_index_chunks(collection, 
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        ids, texts, metadatas, batch_embeddings = [], [], [], []
...
def hybrid_search_vectordb(query: str,
                         ...
                         rerank: bool = True,
                         top_k: int = 5) -> List[Dict]:
    """Hybrid search: vector search, keyword filtering, reranking."""
...
def build_vectordb(source: str = "all", rebuild: bool = False):
    ...
    if collection.count() > 0 and not rebuild:
        ...
        return collection
    load_and_index_chunks(collection, source)
...
def test_search(query: Optional[str] = None):
    """Quick search test for the vector database."""
    client = init_chromadb()
